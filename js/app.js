...
async function createAccount(){
 ...
 if(!name)return renderAuth('Enter your name.'); if(!validUserId(username))return renderAuth('Enter a User ID between 1 and 50 characters.'); if(!email)return renderAuth('Enter your email.'); if(password.length<6)return renderAuth('Password must be at least 6 characters.'); if(password!==confirm)return renderAuth('Passwords do not match.');
 button.disabled=true;button.textContent='Checking User ID...';
 try{
  const usernameRef=ref(db,`usernames/${username}`); const existing=await get(usernameRef); if(existing.exists()){renderAuth(`@${username} is already taken. Choose another User ID.`);return;}
 ...
 }catch(error){try{if(auth.currentUser)await signOut(auth);}catch{}renderAuth(firebaseMessage(error));}
}
