...
function renderReel(reel, index) {
  return `
    <article class="reel-item" data-reel-index="${index}" data-reel-user-id="${reel.creator || reel.userId}">
      ...
      <div class="reel-info">
        <p class="reel-user-id"><button class="reel-user-link" type="button" data-reel-profile="${reel.creator || reel.userId}">${reel.creator || reel.userId}</button></p>
        <p class="reel-caption">${reel.caption || reel.title || ""}</p>
      </div>
      <div class="reel-actions">
        <button class="reel-action" type="button" data-reel-action="like">♡ <span data-like-count>0</span></button>
...
export function renderReelsPage(container) {
  ...
  container.innerHTML = `
    <main class="reels-page">
      <section class="reels-feed" data-reels-feed aria-label="Reels feed">${reels.map(renderReel).join("")}</section>
  ...
  container.addEventListener("click", async (event) => {
    ...
    try {
      if (actionButton.dataset.reelAction === "like") {
        const state = await toggleLike(reel.id);
        actionButton.firstChild.textContent = state.liked ? "♥ " : "♡ ";
        const count = actionButton.querySelector("[data-like-count]");
        if (count) count.textContent = String(state.likes || 0);
      }
