...
(function () {
  if (window.__indoStoryRuntimeV12) return;
  window.__indoStoryRuntimeV12 = true;

  const mobileStyle = document.createElement('style');
  mobileStyle.id = 'indo-mobile-runtime-v6';
  mobileStyle.textContent = `html,body{width:100%;min-height:100%;-webkit-text-size-adjust:100%}body{overflow-x:hidden;overflow-y:auto}button,a,input,textarea,select{touch-action:manipulation;-webkit-tap-highlight-color:transparent}.app-shell,.auth-shell{width:100%;max-width:520px;min-height:100dvh;min-height:100svh}.story-preview{overflow:hidden!important}#story-text-button{position:absolute!important;right:14px!important;bottom:118px!important;z-index:140!important;width:44px!important;height:44px!important;border-radius:50%!important;background:rgba(20,20,27,.88)!important;border:1px solid rgba(255,255,255,.18)!important;color:#fff!important;font-size:18px!important;font-weight:900!important;display:grid!important;place-items:center!important;box-shadow:0 8px 24px rgba(0,0,0,.45)!important;cursor:pointer!important}#story-publish-button{display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important}#indo-story-done-hit{pointer-events:auto!important;position:absolute!important;right:0!important;bottom:0!important;width:20%!important;height:44px!important;z-index:150!important;border:0!important;border-radius:10px!important;background:#7b3cff!important;color:#fff!important;font-weight:800!important;display:block!important;visibility:visible!important;opacity:1!important;cursor:pointer!important}`;
  document.head.appendChild(mobileStyle);

  const warmScreens=()=>{const version='20260813-68';const screens=['./src/screens/home-v2.js','./src/screens/reels.js','./src/screens/create.js','./src/screens/story-create.js','./src/screens/profile-direct.js','./src/screens/settings.js','./src/screens/search.js','./src/screens/notifications.js','./src/screens/activity.js','./src/screens/wallet.js','./src/screens/blocked-users.js'];screens.forEach(path=>import(`${path}?v=${version}`).catch(()=>{}));};
  ...
  async function directPublish(preview,publish,message,done){
    ...
    publishing=true;publish.disabled=true;done.disabled=true;done.textContent='Posting...';message.textContent='Uploading story...';
    try{
      const mod=await import(`../src/features/upload/story-publish.js?v=20260813-68`);
    ...
    }catch(error){console.error('Direct Story publish failed:',error);message.textContent=error?.message||'Story upload failed. Please try again.';publish.disabled=false;done.disabled=false;done.textContent='Done';publishing=false;}
  }
  ...
  function apply(){
    ...
    publish.style.setProperty('display','none','important');publish.style.setProperty('visibility','hidden','important');publish.style.setProperty('opacity','0','important');publish.style.setProperty('pointer-events','none','important');
    ...
    if(!hit||hit.parentElement!==preview){
      ...
      const trigger=e=>{e.preventDefault();e.stopPropagation();const p=document.getElementById('story-preview'),b=document.getElementById('story-publish-button'),m=document.getElementById('story-create-message'),d=document.getElementById('indo-story-done-hit');if(p&&b&&m&&d)void directPublish(p,b,m,d);};
