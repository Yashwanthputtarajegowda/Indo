...
(function () {
  if (window.__indoStoryRuntimeV10) return;
  window.__indoStoryRuntimeV10 = true;

  const mobileStyle = document.createElement('style');
  mobileStyle.id = 'indo-mobile-runtime-v4';
  ...
  const warmScreens=()=>{const version='20260813-66';const screens=['./src/screens/home-v2.js','./src/screens/reels.js','./src/screens/create.js','./src/screens/story-create.js','./src/screens/profile-direct.js','./src/screens/settings.js','./src/screens/search.js','./src/screens/notifications.js','./src/screens/activity.js','./src/screens/wallet.js','./src/screens/blocked-users.js'];screens.forEach(path=>import(`${path}?v=${version}`).catch(()=>{}));};
  ...
  async function directPublish(preview,publish,message){
    ...
    try{
      // This file is loaded by index.html from the site root. Never use ../src here.
      const mod=await import(`./src/features/upload/story-publish.js?v=20260813-66`);
  ...
  // Do not let any generic preview click start text. Text is exclusively opt-in via T.
  const swallowGenericStoryText=(event)=>{
    ...
    if(!window.__indoStoryTextMode&&(event.type==='pointerdown'||event.type==='click')){
