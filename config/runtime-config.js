...
(function () {
  if (window.__indoStoryRuntimeV8) return;
  window.__indoStoryRuntimeV8 = true;

  const mobileStyle = document.createElement('style');
  mobileStyle.id = 'indo-mobile-runtime-v2';
  ...
  async function directPublish(preview,publish,message){
    ...
    try{
      const {publishStory}=await import('../src/features/upload/story-publish.js?v=20260813-64');
  ...
  function apply(){
    ...
    if(!textButton||textButton.parentElement!==preview){
      ...
      const openText=(event)=>{event.preventDefault();event.stopPropagation();const x=preview.getBoundingClientRect().left+preview.getBoundingClientRect().width/2;const y=preview.getBoundingClientRect().top+preview.getBoundingClientRect().height/2; window.__indoStoryStartTitle?.(x,y);};
  ...
  window.__indoStoryStartTitle=(x,y)=>{
    ...
    const event=new MouseEvent('click',{bubbles:true,clientX:x,clientY:y}); preview.dispatchEvent(event);
  };
})();