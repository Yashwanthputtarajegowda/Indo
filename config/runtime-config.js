...
(function () {
  if (window.__indoStoryRuntimeV9) return;
  window.__indoStoryRuntimeV9 = true;

  const mobileStyle = document.createElement('style');
  mobileStyle.id = 'indo-mobile-runtime-v3';
  ...
  function collectStoryEditor(preview) {
    ...
    return {
      title:title?.textContent?.trim()||'', titleFont:title?.dataset?.font||title?.style?.fontFamily||'Arial, sans-serif',
      titleX:Number(title?.dataset?.x||50), titleY:Number(title?.dataset?.y||14), crop,
      stickerDataUrl:photos[0]?.src||'', stickerX:Number(photos[0]?.dataset?.x||50), stickerY:Number(photos[0]?.dataset?.y||50), stickerScale:Number(photos[0]?.dataset?.gestureScale||1),
      elements:[...(title?[{type:'title',text:title.textContent||'',x:Number(title.dataset?.x||50),y:Number(title.dataset?.y||14),font:title.dataset?.font||title.style?.fontFamily||'Arial, sans-serif'}]:[]),...photos.map(node=>({type:'photo',dataUrl:node.src||'',x:Number(node.dataset?.x||50),y:Number(node.dataset?.y||50),scale:Number(node.dataset?.gestureScale||1)})),...emojis.map(node=>({type:'emoji',emoji:node.textContent||'',x:Number(node.dataset?.x||50),y:Number(node.dataset?.y||50)}))]
    };
  }
  ...
  async function directPublish(preview,publish,message){
    ...
    try{
      const {publishStory}=await import('./src/features/upload/story-publish.js?v=20260813-65');
  ...
  function apply(){
    ...
    if(!textButton||textButton.parentElement!==preview){
      ...
      const openText=(event)=>{
        ...
        window.__indoStoryAllowTitleOnce=true;
    ...
    if(!hit||hit.parentElement!==preview){
      ...
      hit.style.cssText='position:absolute;right:0;bottom:0;width:20%;height:44px;z-index:150;cursor:pointer;background:transparent;border:0;padding:0;margin:0;touch-action:manipulation;-webkit-tap-highlight-color:transparent;pointer-events:auto;';
      ...
      hit.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();},true);
      preview.appendChild(hit);
    }
    return true;
  }

  function start(){let attempts=0;const timer=window.setInterval(()=>{attempts+=1;if(apply()||attempts>=100)window.clearInterval(timer);},100);apply();}
  ...
  // Text is intentionally opt-in: only the T button may activate title mode.
  const blockImplicitTitleClicks = (event) => {
    const preview = event.target instanceof Element ? event.target.closest('#story-preview') : null;
    if (!preview) return;
    const target = event.target instanceof Element ? event.target : null;
    if (window.__indoStoryAllowTitleOnce) { window.__indoStoryAllowTitleOnce=false; return; }
    ...
    if (event.type === 'click' || target?.matches('#story-preview-video') || target === preview) {
      event.preventDefault();
      event.stopImmediatePropagation();
    }
  };
  document.addEventListener('pointerdown',blockImplicitTitleClicks,true);
  document.addEventListener('click',blockImplicitTitleClicks,true);

  window.__indoStoryStartTitle=(x,y)=>{
    const preview=document.getElementById('story-preview'); if(!preview)return;
    window.__indoStoryAllowTitleOnce=true;
