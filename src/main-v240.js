import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=240';
import { installProfileLinkNavigation } from './features/ui/profile-link-navigation-v236.js';

const app = document.getElementById('root');
const ROUTER_VERSION = '240';
...
async function render() {
  ...
  const { render } = await import(`./router-v240.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV240) return;
  window.__indoNavigationClicksInstalledV240 = true;
