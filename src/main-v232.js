import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=232';

const app = document.getElementById('root');
const ROUTER_VERSION = '232';
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV232) return;
  window.__indoNavigationClicksInstalledV232 = true;
...
async function render() {
  ...
  const { render } = await import(`./router-v232.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
