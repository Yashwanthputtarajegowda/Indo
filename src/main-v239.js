import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=239';
import { installProfileLinkNavigation } from './features/ui/profile-link-navigation-v236.js';

const app = document.getElementById('root');
const ROUTER_VERSION = '239';
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV239) return;
  window.__indoNavigationClicksInstalledV239 = true;
  document.addEventListener('click', (event) => {
    const element = event.target instanceof Element ? event.target : null;
    const button = element?.closest('[data-screen]');
    if (!button) return;
    const surface = button.closest('.indo-option5-topbar,.indo-global-bottom-nav,.indo-brand-topbar,.bottom-nav,.page-head,.reels-top,.pc239-shell');
    if (!surface) return;
...
async function render() {
  ...
  const { render } = await import(`./router-v239.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
