import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=238';
import { installProfileLinkNavigation } from './features/ui/profile-link-navigation-v236.js';

const app = document.getElementById('root');
const ROUTER_VERSION = '238';
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV238) return;
  window.__indoNavigationClicksInstalledV238 = true;
  document.addEventListener('click', (event) => {
    const element = event.target instanceof Element ? event.target : null;
    const button = element?.closest('[data-screen]');
    if (!button) return;
    const surface = button.closest('.indo-option5-topbar,.indo-global-bottom-nav,.indo-brand-topbar,.bottom-nav,.page-head,.reels-top,.profile-v237-shell');
    if (!surface) return;
    ...
    if (screen === 'profile' && button.hasAttribute('data-own-profile')) {
      ...
      import('./state.js').then(({ state }) => {
        ...
        return render();
      }).then(() => window.scrollTo({ top: 0, behavior: 'auto' })).catch((error) => console.error('Own profile navigation failed:', error));
      return;
    }
    event.preventDefault();
    event.stopPropagation();
...
async function render() {
  ...
  const { render } = await import(`./router-v238.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
...
async function start() {
  ...
  try {
    try {
      ...
      console.warn('Recommendation engine disabled for this boot:', error);
    }
