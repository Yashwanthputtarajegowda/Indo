import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=242';
import { installProfileLinkNavigation } from './features/ui/profile-link-navigation-v236.js';

const app = document.getElementById('root');
const ROUTER_VERSION = '242';
...
async function render() {
  ...
  const { render } = await import(`./router-v242.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV242) return;
  window.__indoNavigationClicksInstalledV242 = true;
