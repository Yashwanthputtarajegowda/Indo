import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=233';
import { installProfileLinkNavigation } from './features/ui/profile-link-navigation-v233.js';

const app = document.getElementById('root');
const ROUTER_VERSION = '233';
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV233) return;
  window.__indoNavigationClicksInstalledV233 = true;
...
async function render() {
  ...
  const { render } = await import(`./router-v233.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
