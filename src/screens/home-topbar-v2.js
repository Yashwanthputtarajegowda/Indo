...
const STYLE_ID='indo-home-topbar-badge-v230';
let pollTimer=null;
let pollBusy=false;
...
async function loadUnreadCount(){
  ...
  try{
    ...
    const response=await fetch(`${window.INDO_API_BASE||''}/api/notifications/unread-count`,{headers:{Authorization:`Bearer ${token}`},cache:'no-store'});
...
function paintUnreadBadge(count){
  ...
  if(n===0){existing?.remove();button.removeAttribute('data-unread-count');button.setAttribute('aria-label','Notifications');return;}
...
async function refreshUnreadBadge(){
  if(pollBusy)return;
  pollBusy=true;
  try{paintUnreadBadge(await loadUnreadCount())}finally{pollBusy=false}
}

function clearUnreadBadge(){paintUnreadBadge(0)}
