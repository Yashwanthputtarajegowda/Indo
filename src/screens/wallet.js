...
function renderPayouts(payouts) {
  ...
  return payouts
    .map(
      (payout) =>
        `<div class="payout-row"><div><strong>$${Number(payout.amount || 0).toFixed(2)}</strong><small>${escapeHtml(payout.method || "manual")} · ${escapeHtml(payout.status || "pending")}</small></div><span>${new Date(Number(payout.createdAt || Date.now())).toLocaleDateString()}</span></div>`,
    )
    .join("");
}
