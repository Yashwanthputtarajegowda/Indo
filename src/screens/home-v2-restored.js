...
function normalizeStory(story) {
  ...
  return { ...story, secureUrl, ownerUid: String(story.ownerUid || story.uid || story.userId || story.creatorUid || '').trim(), username: String(story.username || story.userName || story.handle || story.name || 'Indo User').replace(/^@/, '') };
}
...
function ensureHomeStoryStyles() {
  const id = 'indo-home-story-cards-v2';
  if (document.getElementById(id)) return;
  const style = document.createElement('style');
  style.id = id;
  style.textContent = `.indo-story-row{display:flex;gap:10px;align-items:stretch;overflow-x:auto;overflow-y:hidden;padding:10px 8px 12px;margin:0;border-bottom:1px solid #17171c;scrollbar-width:none;-webkit-overflow-scrolling:touch}.indo-story-row::-webkit-scrollbar{display:none}.indo-story-card{position:relative;display:flex;flex:0 0 118px;min-width:118px;width:118px;height:146px;flex-direction:column;overflow:hidden;padding:0;text-align:left;border:1px solid #262632;border-radius:14px;background:#111117;color:#f4f4f7;cursor:pointer;box-shadow:0 5px 16px rgba(0,0,0,.16)}.indo-story-card-own{border-color:#6f34d7;background:linear-gradient(145deg,#4a23a8 0%,#cf2f9f 100%)}.indo-story-card-media{position:relative;height:80px;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#1a1a23}.indo-story-avatar{width:50px;height:50px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,#7637ff,#f044ae);color:#fff;font-size:20px;font-weight:900}.indo-story-own-bg{width:100%;height:100%;display:grid;place-items:center;background:linear-gradient(135deg,#7736ff,#dd2da8)}.indo-story-plus{width:42px;height:42px;border-radius:12px;display:grid;place-items:center;border:1px solid rgba(255,255,255,.78);background:rgba(255,255,255,.08);font-size:26px;font-weight:300;color:#fff}.indo-story-online{position:absolute;right:7px;top:7px;width:10px;height:10px;border-radius:50%;background:#2ee66b;border:2px solid #111117}.indo-story-card-body{display:flex;min-width:0;flex:1;flex-direction:column;justify-content:center;gap:4px;padding:8px 9px 10px}.indo-story-card-body strong{font-size:12px;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.indo-story-card-body span{font-size:9px;line-height:1.25;color:#b6b6c1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.indo-story-card-own .indo-story-card-body span{color:#f0e4ff}.indo-story-card::after{content:'';position:absolute;left:9px;right:9px;bottom:6px;height:2px;border-radius:99px;background:linear-gradient(90deg,#8439ff,#ee3cab)}.indo-story-viewer{position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,.96);display:grid;place-items:center;padding:18px}.indo-story-card-viewer{position:relative;width:min(100%,420px);height:min(90vh,760px);background:#000;border-radius:16px;overflow:hidden}.indo-story-card-viewer video{width:100%;height:100%;object-fit:contain;background:#000}.indo-story-close,.indo-story-share,.indo-story-more{position:absolute;top:10px;z-index:3;width:34px;height:34px;border:0;border-radius:50%;background:rgba(0,0,0,.55);color:#fff}.indo-story-close{left:12px;font-size:24px}.indo-story-share{right:52px}.indo-story-more{right:12px}.indo-story-title{position:absolute;left:54px;right:90px;top:17px;z-index:2;color:#fff;font-size:13px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.indo-story-menu{position:absolute;right:12px;top:52px;z-index:4;display:none;min-width:150px;padding:6px;border-radius:10px;background:#18181f;border:1px solid #2b2b33}.indo-story-menu.open{display:block}.indo-story-menu button{display:block;width:100%;padding:10px 12px;border:0;background:none;color:#fff;text-align:left;font-weight:700}.indo-story-menu .delete{color:#ff6b6b}@media(min-width:700px){.indo-story-card{flex-basis:132px;min-width:132px;width:132px;height:154px}.indo-story-card-media{height:84px}}`;
...
async function openStoryViewer(story, isOwn = false) {
  ...
  overlay.querySelector('.indo-story-share')?.addEventListener('click', async () => {
    ...
    try { if (navigator.share) await navigator.share({ title: 'Indo story', text: 'Watch this story on Indo', url }); else if (navigator.clipboard?.writeText) await navigator.clipboard.writeText(url); else window.prompt('Copy story link:', url); }
  ...
  if (isOwn) {
    ...
    overlay.querySelector('.delete')?.addEventListener('click', async () => {
      ...
      localStorage.removeItem(LAST_STORY_KEY); close(); loadStories(document.getElementById('root')).catch(() => {});
    });
  }
...
async function openStoryPicker(event) {
  ...
  const input = document.createElement('input'); input.type = 'file'; input.accept = 'video/*'; input.style.position = 'fixed'; input.style.left = '-9999px'; input.style.width = '1px'; input.style.height = '1px';
  document.body.appendChild(input);
  input.addEventListener('change', async () => { const file = input.files?.[0]; input.remove(); if (!file || !file.type.startsWith('video/')) return; window.__indoStoryDraftFile = file; await window.__indoNavigate?.('story-create'); }, { once: true });
  input.click();
}
...
async function loadStories(app) {
  ...
  if (!row) return;
  try {
    const [{ loadStories: fetchStories }, { auth }] = await Promise.all([import('../features/stories/stories.js?v=20260814-127'), import('../features/auth/firebase-client.js')]);
...
async function loadFeed(app) {
  ...
  try {
    const { loadHomeVideos, renderVideoCard, bindVideoCards } = await import('../features/feed/home-feed.js?v=20260814-127');
...
async function loadNotifications(app) {
  try {
    ...
    if (!button) return;
    const { loadNotifications: fetchNotifications } = await import('../features/notifications/notifications.js?v=20260814-127');
    const items = await fetchNotifications(); const unread = items.filter((item) => !item.read).length;
    if (!unread) return;
