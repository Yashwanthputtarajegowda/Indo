...
export function renderHomeTopbar() {
  ...
  return `
    <header class="topbar indo-option5-topbar">
      <button class="indo-option5-brand" type="button" data-screen="home" aria-label="Indo Home" title="Home">
        <span class="indo-option5-mark" aria-hidden="true"><svg viewBox="0 0 32 32" role="img"><path d="M19.7 2 7.3 17.1h7.2L11.8 30 25 13.2h-7.1L19.7 2Z" fill="currentColor"/></svg></span>
      ...
      <div class="top-actions indo-option5-actions" aria-label="Home actions">
        <button class="search-button top-action-button" data-screen="search" aria-label="Search" title="Search">
          <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.8" cy="10.8" r="6.2" fill="none" stroke="currentColor" stroke-width="2"/><path d="m16 16 4.2 4.2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
        </button>
        <button class="notification-button top-action-button" data-screen="notifications" aria-label="Notifications" title="Notifications">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 10a5 5 0 0 1 10 0v4l2 2H5l2-2zM10 19h4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </button>
        <button class="create-button top-action-button indo-option5-create" data-screen="create" aria-label="Create" title="Create">
...
export function installHomeTopbarStyles() {
  ...
  style.textContent = `
    ...
    .indo-option5-brand{min-width:0;height:100%;display:flex;align-items:center;gap:9px;padding:0;margin:0;border:0;background:transparent;color:#fff;cursor:pointer;text-align:left}
    .indo-option5-mark{width:31px;height:35px;flex:0 0 31px;display:grid;place-items:center;color:#ff3cac;filter:drop-shadow(0 0 7px rgba(255,47,173,.28));transform:rotate(-4deg)}
    .indo-option5-mark svg{width:29px;height:32px;display:block}
    .indo-option5-wordmark{display:block;color:#f8f7fb;font-family:Impact,"Arial Black",Arial,sans-serif;font-size:24px;font-weight:950;font-style:italic;line-height:.95;letter-spacing:-1.2px;transform:skew(-6deg);text-shadow:1px 0 0 rgba(255,255,255,.15),0 0 10px rgba(255,255,255,.06)}
    .indo-option5-actions{display:flex;align-items:center;justify-content:flex-end;gap:9px;flex:0 0 auto;padding:0}
    .indo-option5-actions .top-action-button{width:30px;height:30px;min-width:30px;padding:0;margin:0;display:grid;place-items:center;border:0;border-radius:50%;background:transparent;color:#f0edf7;box-shadow:none;transition:transform .16s ease,color .16s ease,background .16s ease;position:relative}
    .indo-option5-actions .top-action-button:hover{color:#ff56c2;background:rgba(255,55,190,.07);transform:translateY(-1px)}
    .indo-option5-actions .top-action-button:active{transform:scale(.92)}
    .indo-option5-actions .top-action-button svg{width:19px;height:19px;display:block;overflow:visible}
    .indo-option5-actions .notification-button svg{width:20px;height:20px}
    .indo-option5-actions .indo-option5-create{width:31px;height:31px;min-width:31px;color:#fff;background:linear-gradient(145deg,#ff3fb7 0%,#d83b9f 45%,#963bff 100%);box-shadow:0 0 0 1px rgba(255,255,255,.08) inset,0 0 13px rgba(240,51,180,.28)}
    .indo-option5-actions .indo-option5-create:hover{color:#fff;background:linear-gradient(145deg,#ff57c1 0%,#e343a8 45%,#a14cff 100%);box-shadow:0 0 0 1px rgba(255,255,255,.14) inset,0 0 18px rgba(240,51,180,.4);transform:translateY(-1px)}
    .indo-option5-actions .indo-option5-create svg{width:18px;height:18px}
    .indo-notification-badge{position:absolute;top:-2px;right:-3px;min-width:15px;height:15px;padding:0 4px;border-radius:999px;background:#ff2caf;color:#fff;display:flex;align-items:center;justify-content:center;font:800 8px/1 Arial,sans-serif;box-shadow:0 0 0 2px #0b0910,0 0 9px rgba(255,44,177,.6);pointer-events:none}
