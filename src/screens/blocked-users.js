...
export async function renderBlockedUsers(app) {
  ...
  try {
    ...
    list.innerHTML = users.map((u) => `<div class="search-user"><div class="avatar small">${escapeHtml((u.name || u.username || 'I').replace(/^@/, '').charAt(0).toUpperCase() || 'I')}</div><div><b>${escapeHtml(u.name || 'Indo User')}</b><small>${escapeHtml(u.username || '')}</small></div><button data-unblock-uid="${escapeHtml(u.uid)}">Unblock</button></div>`).join('');
    list.querySelectorAll('[data-unblock-uid]').forEach((button) => button.addEventListener('click', async () => { button.disabled = true; try { await toggleBlockedUser(button.dataset.unblockUid, false); button.closest('.search-user')?.remove(); if (!list.children.length) list.innerHTML = '<div class="profile-empty">No blocked users.</div>'; } catch (error) { message.textContent = error.message || 'Could not unblock user.'; button.disabled = false; } }));
