...
export function renderLogin(app) {
  app.innerHTML = `<div class="auth-page"><div class="auth-card"><div class="brand auth-brand"><span>♥</span>indo</div><h1>Welcome back</h1><p>Login to continue</p><form id="login-form"><label>User ID<input id="login-user-id" placeholder="@yourid" pattern="@[A-Za-z0-9_]+" required></label><label>Password<input id="login-password" type="password" placeholder="Password" required></label><button class="auth-submit" type="submit">Login</button></form><button class="auth-switch" data-auth="signup">Create a new account</button></div></div>`;
}
