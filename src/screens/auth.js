function authIcon(symbol, extra = '') { return `<span class="auth-icon ${extra}" aria-hidden="true">${symbol}</span>`; }
...
function ambient() {
  return `<div class="auth-ambient" aria-hidden="true">
    <span class="auth-orbit auth-orbit-a"></span>
    <span class="auth-orbit auth-orbit-b"></span>
    <span class="auth-orbit auth-orbit-c"></span>
    <span class="auth-bolt auth-bolt-a">ϟ</span>
    <span class="auth-bolt auth-bolt-b">ϟ</span>
    <span class="auth-star auth-star-a">✦</span>
    <span class="auth-star auth-star-b">•</span>
    <span class="auth-star auth-star-c">✦</span>
    <span class="auth-star auth-star-d">•</span>
  </div>`;
}

function brand(badge, signup = false) {
  return `<header class="auth-brand-block ${signup ? 'auth-signup-brand' : ''}">
    <div class="auth-brand-ring"></div>
    <div class="auth-logo-mark"><span class="auth-logo-thunder">⚡</span><strong>Indo</strong></div>
    <div class="auth-tagline">Connect. Share. <b>Love.</b></div>
    <div class="auth-center-badge ${signup ? 'auth-user-badge' : 'auth-lock-badge'}" aria-hidden="true"><span>⚡</span></div>
  </header>`;
}

export function renderLogin(app) {
  app.innerHTML = `<main class="auth-page indo-auth-v153">${ambient()}<section class="auth-shell" aria-label="Indo login">
    ${brand('⚡')}
    <section class="auth-card auth-login-card">
      <div class="auth-welcome"><div class="auth-kicker">WELCOME TO INDO</div><h1>Welcome back!</h1><p>Login to continue your journey</p></div>
      <form id="login-form" class="auth-form">
        <label class="auth-field"><span>${authIcon('✉')}Email ID</span><div class="auth-input-wrap"><input id="login-email" type="email" placeholder="Enter your email" autocomplete="email" required><b>✉</b></div></label>
...
export function renderSignup(app) {
  app.innerHTML = `<main class="auth-page indo-auth-v153">${ambient()}<section class="auth-shell auth-signup-shell" aria-label="Indo create account">
    ...
    <section class="auth-card auth-signup-card">
      <div class="auth-welcome"><div class="auth-kicker">JOIN THE INDO COMMUNITY</div><h1>Create your account</h1><p>Choose your Indo identity and start <b>sharing</b></p></div>
