...
async function openConversation(conversation) {
  ...
  overlay.innerHTML = `
    <section class="indo-chat">
      <header class="indo-chat-head">
        <button
          class="indo-chat-head-profile"
          type="button"
          data-open-profile="${esc(userId)}"
          data-profile-uid="${esc(uid)}"
          data-profile-username="${esc(userId)}"
        >
          <span
            class="indo-chat-head-avatar"
            data-profile-avatar
            data-profile-uid="${esc(uid)}"
            data-profile-username="${esc(userId)}"
          >
            ${esc(name.charAt(0).toUpperCase() || "I")}
          </span>
          <span class="indo-chat-head-title">
            <b>${esc(name)}</b>
            <span>${esc(userId ? `@${userId}` : "")}</span>
          </span>
        </button>
        <button
          class="indo-chat-close"
          type="button"
          aria-label="Close"
        >
          ×
      ...
      <div
        class="indo-chat-list"
        data-chat-list
      >
        <div
          style="color:#7f8798;font-size:11px;text-align:center;padding:24px"
        >
          Loading...
        </div>
      </div>
      <form class="indo-chat-form" data-chat-form>
        <input
          name="text"
          maxlength="1000"
          autocomplete="off"
  ...
  overlay
    ...
    ?.addEventListener("click", async (event) => {
      ...
      close();
      await window.__indoOpenProfile?.({
        uid,
        userId,
      });
    });
...
export async function renderMessages(app) {
  ...
  app.innerHTML = `
    <div class="indo-msg">
      ...
      <main class="indo-msg-main">
        <div class="indo-msg-title">Messages</div>
        <div
          class="indo-msg-list"
          data-message-list
        >
          <div class="indo-msg-empty">
      ...
      .map((item) => {
        ...
        return `
          <button
            class="indo-msg-card"
            type="button"
            data-conversation-uid="${esc(uid)}"
          >
            <span
              class="indo-msg-avatar"
              data-profile-avatar
              data-profile-uid="${esc(uid)}"
              data-profile-username="${esc(userId)}"
            >
            ...
            <span class="indo-msg-copy">
              <span class="indo-msg-name">
                ${esc(displayName)}
                ${
                  unread
                    ? `<span class="indo-msg-badge">${unread > 99 ? "99+" : unread}</span>`
                    : ""
                }
              </span>
              <span
              ...
              <span class="indo-msg-last">
                ${esc(item.lastMessage || "Start a conversation")}
              </span>
            </span>
            <span class="indo-msg-time">
              ${esc(formatTime(item.lastMessageAt))}
            </span>
          </button>
        `;
      })
    ...
    list
      ...
      .forEach((button) => {
        button.addEventListener("click", (event) => {
          const profileTarget = event.target instanceof Element
            ? event.target.closest("[data-open-profile]")
            : null;
