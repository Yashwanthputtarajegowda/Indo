  ...
  return `
    <header class="topbar">
      ...
      <div class="top-actions">
        <button class="create-top-button" type="button" data-screen="create" aria-label="Create">+</button>
        <button class="search-button" type="button" data-screen="search" aria-label="Search">⌕</button>
        <button class="notification-button" type="button" data-screen="notifications" aria-label="Notifications">♧</button>
      </div>
    </header>`;
}

function ensureStyles() {
  if (document.getElementById('indo-home-top-only-v1')) return;
  const style = document.createElement('style');
  style.id = 'indo-home-top-only-v1';
  style.textContent = `
    .top-actions{display:flex;align-items:center;gap:14px}
    .top-actions button{border:0;background:transparent;color:#fff;cursor:pointer;padding:6px;display:grid;place-items:center;font-size:22px;line-height:1}
    .top-actions .create-top-button{font-size:28px;font-weight:500}
    .top-actions .search-button{font-size:23px}
    .top-actions .notification-button{font-size:23px}
    ...
    .indo-story-add{position:absolute;right:-2px;bottom:-2px;width:20px;height:20px;border-radius:50%;border:2px solid #09090e;background:#7b3cff;color:#fff;font-weight:900;cursor:pointer}
    ...
    .indo-story-close,.indo-story-share,.indo-story-more{position:absolute;top:10px;z-index:3;width:34px;height:34px;border:0;border-radius:50%;background:rgba(0,0,0,.55);color:#fff;cursor:pointer}
    ...
    .indo-story-menu.open{display:block}.indo-story-menu button{display:block;width:100%;padding:10px 12px;border:0;background:none;color:#fff;text-align:left;font-weight:700;cursor:pointer}.indo-story-menu .delete{color:#ff6b6b}
  `;
...
async function openStoryViewer(story, isOwn = false) {
  ...
  overlay.innerHTML = `<div class="indo-story-card">
    <button class="indo-story-close" type="button" aria-label="Close">×</button>
    <button class="indo-story-share" type="button" aria-label="Share">↗</button>
    ${isOwn ? '<button class="indo-story-more" type="button" aria-label="More">⋯</button><div class="indo-story-menu"><button class="delete" type="button">Delete story</button></div>' : ''}
  ...
  if (isOwn) {
    ...
    overlay.querySelector('.delete')?.addEventListener('click', async () => {
      try {
        ...
        localStorage.removeItem(LAST_STORY_KEY);
        close();
        await loadStories(document.getElementById('root'));
      } catch (error) {
        if (menu) menu.textContent = error?.message || 'Could not delete story.';
      }
    });
  }
...
function addYourStory() {
  return `<button class="indo-story-item" type="button" data-story-add aria-label="Add your story"><span class="indo-story-avatar">+</span><span class="indo-story-name">Your story</span></button>`;
}
...
async function loadStories(app) {
  ...
  try {
    const [{ loadStories: fetchStories }, { auth }] = await Promise.all([
      import('../features/stories/stories.js?v=20260813-121'),
    ...
    row.innerHTML = (own ? storyItem(own, true) : addYourStory()) + others.map((item) => storyItem(item, false)).join('');
    bindStories(app);
  } catch (error) {
    console.warn('Stories unavailable:', error);
    row.innerHTML = addYourStory();
    bindStories(app);
  }
}
...
async function loadFeed(app) {
  ...
  try {
    const { loadHomeVideos, renderVideoCard, bindVideoCards } = await import('../features/feed/home-feed.js?v=20260813-121');
...
async function loadNotifications(app) {
  try {
    ...
    if (!button) return;
    const { loadNotifications: fetchNotifications } = await import('../features/notifications/notifications.js?v=20260813-121');
...
export function renderHome(app) {
  ...
  app.innerHTML = `<div class="app-shell">
    ${renderTopbar()}
    <div class="indo-story-row" data-stories></div>
    <main class="feed">
      <div class="feed-status" data-feed-status>Loading videos...</div>
      <div data-home-feed></div>
    </main>
  </div>`;
  loadStories(app);
  loadFeed(app);
