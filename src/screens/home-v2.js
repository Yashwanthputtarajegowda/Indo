...
function ensureStoryStyles() {
  if (document.getElementById('indo-story-inline-v4')) return;
  const style = document.createElement('style');
  style.id = 'indo-story-inline-v4';
  style.textContent = `
    ...
    .story-v2{position:relative!important;display:flex!important;flex:0 0 66px!important;flex-direction:column!important;align-items:center!important;justify-content:flex-start!important;width:66px!important;min-width:66px!important;max-width:66px!important;height:auto!important;min-height:72px!important;padding:0!important;margin:0!important;gap:6px!important;color:#d8d8df!important;text-align:center!important;border:0!important;background:none!important;white-space:normal!important;cursor:pointer!important;}
    .story-v2-avatar{position:relative!important;display:grid!important;place-items:center!important;width:56px!important;height:56px!important;min-width:56px!important;max-width:56px!important;flex:0 0 56px!important;border-radius:50%!important;background:linear-gradient(135deg,#743cff,#f83ab8)!important;color:#fff!important;font-size:15px!important;font-weight:800!important;margin:0 auto!important;overflow:visible!important;}
    ...
    .story-v2-add{position:absolute!important;right:-2px!important;bottom:-2px!important;width:20px!important;height:20px!important;border-radius:50%!important;border:2px solid #09090e!important;background:#7b3cff!important;color:#fff!important;font-size:13px!important;line-height:16px!important;font-weight:900!important;padding:0!important;display:grid!important;place-items:center!important;z-index:3!important;}
    ...
    .story-viewer-v2-close{position:absolute;right:12px;top:10px;z-index:2;width:34px;height:34px;border:0;border-radius:50%;background:rgba(0,0,0,.55);color:#fff;font-size:24px;line-height:30px;cursor:pointer;}
...
function storyItem(story, isOwn = false) {
  ...
  const safeId = String(story.id || '');
  return `<div class="story-v2" data-story-open data-story-url="${escapeHtml(storyUrl)}" data-story-name="${escapeHtml(username)}" data-story-id="${escapeHtml(safeId)}">
    <span class="story-v2-avatar">${initial}${isOwn ? '<button class="story-v2-add" type="button" data-story-add aria-label="Add another story">+</button>' : ''}</span>
    <span class="story-v2-name">@${username}</span>
  </div>`;
}
...
function bindStoryInteractions(app) {
  app.querySelectorAll('[data-story-add]').forEach((button) => {
    button.addEventListener('click', async (event) => {
      ...
      await (await import('../main.js?v=20260813-32')).navigate?.('story-create');
    });
  });
...
async function loadStories(app) {
  ...
  try {
    ...
    if (ownStories.length) {
      const latestOwn = ownStories[0];
      row.innerHTML = storyItem(latestOwn, true) + otherStories.map((story) => storyItem(story, false)).join('');
    } else {
      row.innerHTML = addStoryItem() + otherStories.map((story) => storyItem(story, false)).join('');
    }
