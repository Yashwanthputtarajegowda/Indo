...
async function loadFeed(app) {
  ...
  try {
    const { loadHomeVideos, renderVideoCard, bindVideoCards } = await import('../features/feed/home-feed.js?v=20260813-19');
...
async function loadStories(app) {
  ...
  try {
    const { loadStories } = await import('../features/stories/stories.js?v=20260813-19');
...
async function loadNotifications(app) {
  try {
    ...
    if (!button) return;
    const { loadNotifications } = await import('../features/notifications/notifications.js?v=20260813-19');
