...
function card(item) { const thumb = item.thumbnailUrl; const img = thumb ? `<img class="indo-community-video-thumb" src="${esc(thumb)}" alt="" loading="lazy" decoding="async">` : `<div class="indo-community-video-fallback">▶</div>`; return `<article class="indo-community-video-card" data-watch-video-id="${esc(item.id)}"><div class="indo-community-video-thumb-wrap">${img}<span class="indo-community-video-play">▶</span></div><div class="indo-community-video-body"><div style="display:flex;justify-content:space-between;gap:8px"><h3 class="indo-community-video-title">${esc(item.title || "Indo video")}</h3><span class="indo-community-video-badge">COMMUNITY</span></div><div class="indo-community-video-meta"><span>@${esc(String(item.creatorName || item.username || "user").replace(/^@/, ""))}</span><span>·</span><span>${esc(age(item.createdAt))}</span></div></div></article>`; }
function isPriorityFollowerVideo(item, followingIds) { const ownerUid = String(item.ownerUid || item.creatorUid || item.uid || "").trim(); return Boolean(ownerUid) && followingIds.has(ownerUid); }
function trendingScore(item) { const likes = Math.max(0, Number(item.likes || 0)); const views = Math.max(0, Number(item.views || 0)); const ageHours = Math.max(0, (Date.now() - Number(item.createdAt || Date.now())) / 3600000); const recency = 1 / Math.max(1, Math.sqrt(ageHours + 1)); return (likes * 5 + views) * (0.7 + 0.3 * recency); }
