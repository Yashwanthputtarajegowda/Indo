...
const ROUTER_VERSION = '20260814-141';
...
function installNavigationClicks(){
  ...
  document.addEventListener('click',(event)=>{
    ...
    if(!button)return;
    const surface=button.closest('.indo-global-bottom-nav,.indo-option5-topbar,.bottom-nav,.page-head,.reels-top');
    if(!surface)return;
...
async function start(){
  ...
  }catch(error){console.error('Indo startup failed:',error);app.innerHTML='<main class="splash-screen splash-error"><div class="splash-logo">I</div><div class="splash-name">Indo</div><p>Indo could not start.</p></main>';}
}
