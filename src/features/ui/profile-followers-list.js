...
async function loadRelation(root,relation){
  ...
  let username=String(profileState.username||root.querySelector('.profile-direct-head h2')?.textContent||'').trim().replace(/^@/,'');
  ...
  if(!targetUid&&username){
    try{
      ...
      const data=await response.json().catch(()=>({}));
      if(response.ok&&data.profile){targetUid=String(data.profile.uid||data.profile.ownerUid||'');username=String(data.profile.username||username);state.profile={...(state.profile||{}),...data.profile,uid:targetUid,username};}
    }catch{}
  }

  if(!targetUid) throw new Error('Profile UID is missing.');
  ...
  try{const direct=await readFirebaseRelation(targetUid,relation,idToken);if(direct.length)return direct;}catch{}

  try{const backend=await readBackendRelation(apiBase,targetUid,relation,idToken);if(backend.length)return backend;}catch{}

  if(profileState[relation]){const local=mapEntries(profileState[relation]);if(local.length)return local;}

  if(username){
    try{
      const response=await fetch(`${apiBase}/api/account/profile/${encodeURIComponent(username)}?t=${Date.now()}`,{headers:{Authorization:`Bearer ${idToken}`},cache:'no-store'});
      const data=await response.json().catch(()=>({}));
      if(response.ok&&data.profile){const local=mapEntries(data.profile[relation]);if(local.length)return local;}
    }catch{}
  }
...
function openList(root,relation){
  ...
  loadRelation(root,relation).then((items)=>{
    if(!items.length){list.innerHTML='<div class="indo-rel-empty">No users yet.</div>';return;}
    list.innerHTML=items.map((item)=>{
      const id=String(item.userId||'').replace(/^@/,'');const name=String(item.name||'Indo User');const initial=(name.trim().charAt(0)||id.charAt(0)||'U').toUpperCase();
    ...
    list.querySelectorAll('[data-rel-uid]').forEach((button)=>button.addEventListener('click',async()=>{const uid=button.dataset.relUid||'';const userId=button.dataset.relUser||'';closeModal();state.profile={uid,ownerUid:uid,username:userId};state.screen='profile';if(window.__indoNavigate)await window.__indoNavigate('profile');}));
  }).catch((error)=>{list.innerHTML=`<div class="indo-rel-empty">${esc(error?.message||'Could not load list.')}</div>`;});
}

function install(){
  if(globalThis[KEY])return;globalThis[KEY]=true;styleOnce();
  document.addEventListener('click',(event)=>{
    const stat=event.target instanceof Element?event.target.closest('.profile-direct-stats > div'):null;if(!stat)return;
    const stats=stat.parentElement;const root=document.getElementById('root');if(!root?.contains(stat)||!stats?.classList.contains('profile-direct-stats'))return;
    const index=Array.prototype.indexOf.call(stats.children,stat);if(index!==1&&index!==2)return;
    event.preventDefault();event.stopImmediatePropagation();openList(root,index===1?'followers':'following');
  },true);
}
