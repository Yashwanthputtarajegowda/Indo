...
function styleOnce() {
  if (document.getElementById("indo-profile-followers-list-style-v5")) return;
...
async function loadRelation(root, relation) {
  ...
  try {
    const direct = await readFirebaseRelation(targetUid, relation, idToken);
...
function install() {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      openList(root, index === 1 ? "followers" : "following");
    },
    true,
  );
}
