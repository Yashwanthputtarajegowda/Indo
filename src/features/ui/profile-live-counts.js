...
async function refresh() {
  ...
  if (!root) return;
  const followersCount = root.querySelector(
    "[data-followers-count]",
  );
  const followingCount = root.querySelector(
    "[data-following-count]",
  );
  const postsCount = root.querySelector("[data-posts]");
  if (!followersCount && !followingCount && !postsCount)
    return;
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        data.error ||
          `Profile sync failed (${response.status}).`,
      );

    const stats = data.stats || data.social || {};
    if (followersCount)
      followersCount.textContent = String(
        Number(stats.followersCount || 0),
      );
    if (followingCount)
      followingCount.textContent = String(
        Number(stats.followingCount || 0),
      );
    if (postsCount)
      postsCount.textContent = String(
    ...
    if (
      state.profile &&
      String(
        state.profile.uid || state.profile.ownerUid || "",
      ) === targetUid
    ) {
      state.profile = {
        ...state.profile,
    ...
    console.warn(
      "Canonical profile refresh failed:",
      error,
    );
  }
}
