...
function closeRelation() {
  document.querySelector('.indo-auth-relations-hardener')?.remove();
}
...
async function openRelation(relation, targetUid) {
  ...
  overlay.addEventListener('click', (event) => { if (event.target === overlay) closeRelation(); });
  ...
  try {
    ...
    if (!items.length) { list.textContent = 'No users yet.'; return; }
    list.style.textAlign = 'left';
    list.innerHTML = items.map((item) => {
      const uid = String(item.uid || '').trim();
      const username = String(item.userId || item.username || '').replace(/^@/, '');
      const name = String(item.name || 'Indo User');
      const initial = (name.trim().charAt(0) || username.charAt(0) || 'U').toUpperCase();
    ...
    list.querySelectorAll('[data-hardener-uid]').forEach((button) => button.addEventListener('click', async () => {
      const uid = String(button.dataset.hardenerUid || '').trim();
      const username = String(button.dataset.hardenerUser || '').trim();
      closeRelation();
    ...
    list.textContent = error?.message || 'Could not load list.';
  }
}
...
function install() {
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (target.matches('.profile-direct-stat[data-relation]')) {
    ...
    if (target.matches('[data-follow]')) {
      ...
      event.preventDefault();
      event.stopImmediatePropagation();
      const next = !target.classList.contains('following');
      target.disabled = true;
      try {
        const data = await api('/api/social/follow', { method:'POST', body:JSON.stringify({ targetUid:uid, follow:next }) });
