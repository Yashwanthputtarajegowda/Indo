...
const STYLE_ID = 'indo-profile-link-navigation-v235';
let installed = false;
let busy = false;
...
function extractUserId(element) {
  ...
  const explicit = element.getAttribute('data-user-id')
...
function findIdentityTarget(element) {
  ...
  const directSelectors = [
    ...
    '.search-user-copy', '.search-user',
    '.notice p b', '.notice',
    '.indo-comment-name', '.indo-comment',
    '.profile-relation-row',
    '.neon-edge-creator', '.neon-edge-name',
    '.indo-story-card-body strong', '.indo-story-card',
    '.watch-creator', '.creator-user', '.post-creator', '.creator',
    '.video-creator', '.video-creator-name', '.video-author',
    '.profile-direct-user-link', '.profile-user-id', '.profile-username',
    '[class*="user-id"]', '[class*="username"]', '[class*="creator"]', '[class*="author"]'
  ];
...
export function installProfileLinkNavigation() {
  ...
  document.addEventListener('click', (event) => {
    ...
    const identityButton = targetElement?.matches('.search-profile-main,.search-profile-card,.notice,.profile-relation-row,.indo-story-card,.search-user') ||
