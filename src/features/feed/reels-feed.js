...
export function renderReel(video) {
  ...
  const fallbackMediaUrl = String(video.videoUrl || video.secureUrl || video.url || "").trim();

  return `<article class="reel-view" data-video-id="${id}" data-source="${escapeHtml(video.source || "")}">
    <video class="reel-video" src="${escapeHtml(mediaUrl)}" data-fallback-src="${escapeHtml(fallbackMediaUrl)}" autoplay muted loop playsinline preload="metadata"></video>
