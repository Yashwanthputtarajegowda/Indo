...
async function openRelation(relation, userId) {
  ...
  window.__indoProfileRelationContext = {
    relation,
    targetUid: String(data.profile.uid || "").trim(),
    returnProfile: {
      userId: String(
        data.profile.userId ||
      ...
      uid: String(data.profile.uid || "").trim(),
    },
  };
...
if (!window.__indoProfileRelationHandlerBound) {
  window.__indoProfileRelationHandlerBound = true;
