...
const USER_CONTAINERS = [
  '.indo-story-card', '.search-profile-card', '.indo-notice-card', '.indo-comment',
  '.indo-rel-v7-row', '.indo-rel-row', '.follower-row', '.following-row', '.user-row',
  '.user-card', '.profile-card', '.conversation', '.message', '.creator-card', '.author-card',
  '.profile-link', '.post-card', '.reel-user', '.indo-watch-creator', '.watch-creator',
  '[data-profile-card]'
].join(',');
...
function textIdentity(el) {
  ...
  const selectors = [
    '.search-profile-id', '.indo-notice-line b', '.indo-comment-name', '.indo-watch-creator-name',
    '.indo-rel-v7-id', '.indo-rel-id', '[class*="user-id"]', '[class*="username"]',
...
function avatarHosts(root) {
  const selectors = [
    ...
    '[data-profile-avatar]', '[data-user-avatar]'
  ];
  const out = [];
  for (const selector of selectors) {
...
function paintIdRow(container, profile) {
  ...
  if (!String(idNode.textContent || '').trim().startsWith('@')) return;
...
async function hydrateContainer(container, force = false) {
  if (!(container instanceof Element)) return;
  const identity = datasetIdentity(container) || (() => {
...
export function invalidateProfileAvatar(uid, userId) { invalidate(uid, userId); }
