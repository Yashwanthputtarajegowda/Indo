...
export async function loadProfileMedia(targetProfile = null) {
  ...
  const requestedUid = String(targetProfile?.uid || targetProfile?.userId || targetProfile?.ownerUid || "").trim();
  ...
  const videos = Array.isArray(data.videos) ? data.videos.filter((video) => String(video.ownerUid || "") === targetUid) : [];
