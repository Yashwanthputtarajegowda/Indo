...
export async function loadNotifications() {
  ...
  const response = await fetch(`${apiBase}/api/notifications`, { headers, cache: 'no-store' });
...
export async function loadUnreadCount() {
  const { apiBase, headers } = await getAuthContext();
  const response = await fetch(`${apiBase}/api/notifications/unread-count`, { headers, cache: 'no-store' });
  const data = await response.json().catch(() => ({}));
