...
const SCREEN_MODULES = {
  ...
  "reel-create": ["./screens/reel-create.js?v=20260815-reel-create-v6", "renderReelCreate"],
...
function ensureUniversalNav(app) {
  if (!app || ["auth-login", "auth-signup", "edit-profile", "watch-video", "report", "reel-create"].includes(state.screen)) return;
  installNavStyles();
...
export async function render(app) {
  try {
    switch (state.screen) {
      ...
      case "settings": await lazy(app, "settings", SCREEN_MODULES.settings[0], SCREEN_MODULES.settings[1], [state.accountType, state.earning, state.earningSummary]); break;
      case "search": await lazy(app, "search", SCREEN_MODULES.search[0], SCREEN_MODULES.search[1]); break;
      case "notifications": await lazy(app, "notifications", SCREEN_MODULES.notifications[0], SCREEN_MODULES.notifications[1]); break;
