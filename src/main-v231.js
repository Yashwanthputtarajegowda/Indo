import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=231';

const app = document.getElementById('root');
const ROUTER_VERSION = '231';
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV231) return;
  window.__indoNavigationClicksInstalledV231 = true;
...
async function render() {
  ...
  const { render } = await import(`./router-v231.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
