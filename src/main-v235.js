...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV235) return;
  window.__indoNavigationClicksInstalledV235 = true;
...
async function render() {
  ...
  const { render } = await import(`./router-v235.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
