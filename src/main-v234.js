import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=234';

const app = document.getElementById('root');
const ROUTER_VERSION = '234';
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV234) return;
  window.__indoNavigationClicksInstalledV234 = true;
...
async function render() {
  ...
  const { render } = await import(`./router-v234.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
