import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=253';
...
async function render(){
  ...
  const {render}=await import(`./router-v253.js?v=${V}`);
  await render(app);
...
if(!window.__indoUniversalNavigationV253){
  window.__indoUniversalNavigationV253=true;
  document.addEventListener('click',(event)=>{
    ...
    if(screen==='profile'&&button.hasAttribute('data-own-profile')){
      import('./state.js').then(({state})=>{
        state.profile=null;state.screen='profile';return render();
      }).then(()=>window.scrollTo({top:0,behavior:'auto'})).catch(console.error);
      return;
    }
