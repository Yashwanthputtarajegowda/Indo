import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=237';
import { installProfileLinkNavigation } from './features/ui/profile-link-navigation-v236.js';

const app = document.getElementById('root');
const ROUTER_VERSION = '237';
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV237) return;
  window.__indoNavigationClicksInstalledV237 = true;
...
async function render() {
  ...
  const { render } = await import(`./router-v237.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
