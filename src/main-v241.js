import { bindAuthSwitches, bindLoginForm, bindSignupForm } from './features/auth/auth-controller.js?v=241';
import { installProfileLinkNavigation } from './features/ui/profile-link-navigation-v236.js';

const app = document.getElementById('root');
const ROUTER_VERSION = '241';
...
async function render() {
  ...
  const { render } = await import(`./router-v241.js?v=${ROUTER_VERSION}`);
  await render(app);
  bindAuthSwitches();
  bindLoginForm();
  bindSignupForm();
...
function installNavigationClicks() {
  if (window.__indoNavigationClicksInstalledV241) return;
  window.__indoNavigationClicksInstalledV241 = true;
