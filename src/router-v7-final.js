...
import { install as installProfileRelations } from './features/ui/profile-relations-v7.js';
const VERSION='20260813-109';
...
function importOne(modulePath){return import(`${modulePath}?v=${VERSION}`).catch(()=>undefined);}
export function preloadAppScreens(){if(preloadStarted)return;preloadStarted=true;const idle=window.requestIdleCallback||((cb)=>window.setTimeout(cb,900));idle(async()=>{for(const modulePath of PRELOAD_BATCH){await importOne(modulePath);await new Promise((resolve)=>window.setTimeout(resolve,120));}}, {timeout:1800});}
...
if(state.screen==='home'){await renderLazy(app,'./screens/home-v2.js','renderHome');importOne('./features/stories/story-stack-enhancer.js').then((enhancer)=>enhancer?.enhanceStoryRow?.(app)).catch((error)=>console.warn('Story enhancer preload failed:',error));return;}
