...
async function probeVideo(url) {
  ...
  try {
    const response = await fetch(url, { method: "HEAD", redirect: "follow", signal: controller.signal, headers: { Accept: "video/*,*/*;q=0.8", "User-Agent": "IndoVideoProxy/1.0" } });
    return { ok: response.ok, url: response.url || url.toString(), contentType: String(response.headers.get("content-type") || "").toLowerCase(), length: Number(response.headers.get("content-length") || 0), acceptRanges: String(response.headers.get("accept-ranges") || "").toLowerCase() };
  } catch { return { ok: true, url: url.toString(), contentType: "" }; }
  finally { clearTimeout(timeout); }
}

function setStreamHeaders(res, mimeType, length, range) {
  res.set({ "Cache-Control": "public, max-age=300, s-maxage=300, stale-while-revalidate=60", "Accept-Ranges": "bytes", "Content-Disposition": "inline", "X-Content-Type-Options": "nosniff", "Content-Type": mimeType || "video/mp4" });
...
export function createExternalVideoLinksRouter({ db, requireUser }) {
  ...
  router.post("/media/external-videos", async (req, res) => {
    ...
    try {
      ...
      if (contentType && !contentType.startsWith("video/") && !/\.(mp4|webm|ogg|ogv|m3u8)(?:$|[?#])/i.test(url.pathname)) return res.status(400).json({ ok: false, error: "That URL is not a supported direct media URL." });
      ...
      const video = { id: videoId, ownerUid: user.uid, mediaType, title, caption, description: caption, creator: profile.username || `@${String(user.uid).slice(0, 8)}`, creatorName: profile.name || "Indo User", sourceUrl: url.toString(), videoUrl: streamUrl, secureUrl: streamUrl, streamUrl, sourceProvider: new URL(probe.url || url).hostname, mimeType: contentType || "video/mp4", size: probe.length || 0, duration: 0, width: 0, height: 0, views: 0, likes: 0, comments: 0, shares: 0, saves: 0, createdAt: Date.now(), storage: { provider: "external-url", mode: "link-only" }, external: { provider: "external-url", sourceUrl: url.toString(), resolvedUrl: probe.url || url.toString(), contentType: contentType || "", acceptRanges: probe.acceptRanges || "" } };
  ...
  const streamHandler = async (req, res) => {
    ...
    const videoId = text(req.params.videoId, 200); if (!videoId) return res.status(400).end();
    try {
      ...
      const requestHeaders = { Accept: "video/*,*/*;q=0.8", "User-Agent": "IndoVideoProxy/1.0" };
      const requestedRange = String(req.headers.range || "").trim();
      if (requestedRange) requestHeaders.Range = requestedRange;
      const upstream = await fetch(source, { method: "GET", redirect: "follow", headers: requestHeaders });
      ...
      const mimeType = String(upstream.headers.get("content-type") || video.mimeType || "video/mp4").split(";")[0].trim() || "video/mp4";
      const contentLength = Number(upstream.headers.get("content-length") || 0);
      const contentRange = String(upstream.headers.get("content-range") || "");
      const isPartial = upstream.status === 206 || Boolean(contentRange);
      setStreamHeaders(res, mimeType, contentLength, contentRange);
      if (req.method === "HEAD") return res.status(isPartial ? 206 : 200).end();
